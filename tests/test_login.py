...
def test_verify_products_are_added_to_cart(setup_page, base_url, credentials):
    ...
    cart=CartPage(setup_page)
    cart.is_loaded()
    added_product_locator_list=setup_page.locator("li.items.even.ng-star-inserted")
    expect(added_product_locator_list).to_have_count(1)
    buyNow_button=added_product_locator_list.get_by_role("button", name="Buy Now")
    buyNow_button.click()
